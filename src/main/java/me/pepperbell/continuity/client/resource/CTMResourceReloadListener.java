...
/**
 * Resource reload listener that replaces BakedModelManagerMixin.
 * This runs during resource reload to load CTM properties and set up quad processors.
 */
public class CTMResourceReloadListener implements SimpleSynchronousResourceReloadListener {
	...
	public void reload(ResourceManager manager) {
		try {
			// Load CTM properties
			CtmPropertiesLoader.LoadingResult result = CtmPropertiesLoader.loadAllWithState(manager);
			...
			boolean wrapEmissive = false; // TODO: Check emissive config
			ModelWrappingHandler.setInstance(wrapCtm, wrapEmissive);
			
			ContinuityClient.LOGGER.info(ContinuityClient.LOG_PREFIX + "Loaded {} CTM properties, wrapping enabled: {}", processorHolders.size(), wrapCtm);
		} catch (Exception e) {
			ContinuityClient.LOGGER.error(ContinuityClient.LOG_PREFIX + "Failed to load CTM properties", e);
		}
	}
}
